...
module.exports.getAuthToken = function(config, agentName, socket) {
    ...
    return new Promise(function(resolve) {
        request.post(url, { body: postdata, json: true, headers: { 'user-agent': config.UA } }, function(err, res, body) {
            if (err) {
                log('Auth error: ' + err);
                debug(err);
            ...
            if (body.apiserver && body.apiserver !== config.LOGDNA_APIHOST) {
                ...
                return resolve(exports.getAuthToken(config, agentName, socket));
            }
